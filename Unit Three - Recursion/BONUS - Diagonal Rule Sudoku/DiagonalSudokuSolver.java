...
public class DiagonalSudokuSolver {
  public static void main(String [] args) {
    ...
    System.out.println();
    System.out.println("Welcome to the Diagonal Sudoku Solver!");
    System.out.println();
  ...
  public static void printPuzzle(int[][] thePuzzle, int numRows, int numCols) {
    ...
    for (int r = 0; r < numRows; r++) {
      for (int c = 0; c < numCols; c++) {
        // Adjust output if values are greater than 9, example is the hexoduko puzzle 
        if (thePuzzle[r][c] > 9) {
          // Use reverse operations of how the vector values were written to puzzle and cast to char
          System.out.printf("%3c", (char)(thePuzzle[r][c] + 'A' - 10));
        }
        else {    // Print values 0-9
  ...
  public static boolean isValidPuzzle(int[][] thePuzzle, int puzzleSize) {
    // Adding arrays of type boolean as a 'flag' to signal if a duplicate is found in the diagonal checks
    boolean[] leftDiagonalFlag = new boolean[puzzleSize * 2];
    boolean[] rightDiagonalFlag = new boolean[puzzleSize * 2];

    // Check if duplicate value exists in each row
    for (int r = 0; r < puzzleSize; r++) {
      // Using an array of type boolean as a 'flag' to signal if a duplicate is found
      boolean[] flag = new boolean[puzzleSize * 2];
      for (int c = 0; c < puzzleSize; c++) {
    ...
    // Check if duplicate value exists in long diagonal starting from top left
    for (int i = 0; i < puzzleSize; i++) {
      int currentValue = thePuzzle[i][i];
      // Ignore the empty spaces
      if (currentValue == -1) {
        continue;
      }
      else {
        if (leftDiagonalFlag[currentValue] == false) {    // If number hasn't occurred, value in flag array will be false 
          leftDiagonalFlag[currentValue] = true;    // Mark value in flag array as true so future iterations can catch duplicates
	}
	else {
	  return false;    // Duplicate was found in row, return false
        }
    ...
    // Check if duplicate value exists in long diagonal starting from top right
    for (int i = 0; i < puzzleSize; i++) {
      int currentValue = thePuzzle[i][puzzleSize - 1 - i];
      // Ignore the empty spaces
      if (currentValue == -1) {
        continue;
      }
      else {
        if (rightDiagonalFlag[currentValue] == false) {    // If number hasn't occurred, value in flag array will be false 
          rightDiagonalFlag[currentValue] = true;    // Mark value in flag array as true so future iterations can catch duplicates
	}
	else {
	  return false;    // Duplicate was found in row, return false
        }
